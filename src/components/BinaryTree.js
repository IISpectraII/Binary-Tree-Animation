/**
 * Componente de Arbol Binario con animaciones
 * Código fuente por Luca Sbardella. Usuario: Isbardel
 * Pagina en Github: https://gist.github.com/lsbardel
 * Recuperado de: https://gist.github.com/lsbardel/83ea5911884ae2773991eda6c11856b2?fbclid=IwAR1knsuWYQuCTirvFBdSPXWPPKoUztAAiKd8h1vWyJMYGqiYL9Vti_WDFmQ
 * Modificado para Reto para el parcial #2
 */

import React, { Component } from "react";
import * as d3 from "d3";
const bt = require("d3-binarytree");

export default class BinaryTree extends Component {  
  renderTree() {
    const originalInsert = bt.binaryTree.prototype.insert;
    
    bt.binaryTree.prototype.insert = function(node) {
      if (!this.root) {
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3
          .forceX(function(nd) {
            if (nd.parent) {
              const x1 = nd.parent.parent ? nd.parent.parent.x : 0.5;
    ...
    function draw(type, r) {
      const example = d3.select("#example");
      const width = 1080;
  ...
  render() {
    return (
      <div ref="paper" className="container-fluid">
        <h1 className="align-center"> Hola Mundo! </h1>
        <div ref="example"></div>
      </div>
    );
  }
